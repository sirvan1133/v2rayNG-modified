    ...
    fun updateConfigViaSub(it: SubscriptionCache): SubscriptionUpdateResult {
        try {
            ...
            try {
                ...
                val resp = HttpUtil.getUrlContentWithHeaders(
                    UrlContentRequest(
                        url = url,
                        userAgent = userAgent,
                        timeout = 15000,
            ...
            if (configText.isEmpty()) {
                try {
                    ...
                    LogUtil.e(AppConfig.TAG, "Failed to get URL content with user agent", e)
                }
            }
            if (configText.isEmpty()) {
                return SubscriptionUpdateResult(failureCount = 1)
            ...
            responseHeaders?.let { headers ->
                LogUtil.d(AppConfig.TAG, "SubUpdate: headers=$headers")
                val userInfo = headers.entries.firstOrNull { it.key.equals("Subscription-Userinfo", ignoreCase = true) }?.value
                LogUtil.d(AppConfig.TAG, "SubUpdate: Subscription-Userinfo=$userInfo")
                if (userInfo != null) {
                    for (part in userInfo.split(";")) {
                        val kv = part.trim().split("=", limit = 2)
                        if (kv.size != 2) continue
                        when (kv[0].trim()) {
                            "upload" -> it.subscription.trafficUpload = kv[1].trim().toLongOrNull() ?: 0
                            "download" -> it.subscription.trafficDownload = kv[1].trim().toLongOrNull() ?: 0
                            "total" -> it.subscription.trafficTotal = kv[1].trim().toLongOrNull() ?: 0
                            "expire" -> it.subscription.trafficExpire = kv[1].trim().toLongOrNull() ?: 0
                        }
                    }
                }
            }
